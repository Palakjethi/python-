...
class ImageToPDFConvertor:
  ...
  def convert_images_to_pdf(self):
    ...
    for widget in self.root.winfo_children():
      if isinstance(widget, tk.Button) and widget  ["text"] == "Convert to PDF":
        widget.config(bg="green", fg="white")
        self.root.update_idletasks()

    output_pdf_path=self.output_pdf_name.get() +".pdf" if self.output_pdf_name.get() else "ouput.pdf"
    ...
    messagebox.showinfo("Success","Conversion to PDF Completed!")    
